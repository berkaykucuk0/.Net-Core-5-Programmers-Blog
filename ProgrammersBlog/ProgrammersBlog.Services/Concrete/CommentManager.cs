...
namespace ProgrammersBlog.Services.Concrete
{
    public class CommentManager:ICommentService
    {
        ...
        {
            var comment = await _unitOfWork.Comments.GetAsync(c => c.Id == commentId , c=>c.ArticleId);
            if (comment != null)
            {
        ...
        public async Task<IDataResult<CommentDto>> ApproveAsync(int commentId,string modifiedByName)
        {
            ...
            if (comment!=null)
            {
                ...
                comment.ModifiedDate = DateTime.Now;
                var updatedComment = await _unitOfWork.Comments.UpdateAsync(comment);
                article.CommentsCount = await _unitOfWork.Comments.CountAsync(c => c.ArticleId == article.Id && !article.IsDeleted);
                await _unitOfWork.Articles.UpdateAsync(article);
                await _unitOfWork.SaveAsync();
                return new DataResult<CommentDto>(ResultStatus.Success,Messages.Comment.Approve(comment.ModifiedByName),new CommentDto { Comment=comment});
